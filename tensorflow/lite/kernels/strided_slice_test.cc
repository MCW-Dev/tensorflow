...
// Casts input vector to specified type, converting to string for std::string type.
template <typename T>
...
using DataTypes =
    ::testing::Types<float, Eigen::half, Eigen::bfloat16, uint8_t, uint32_t, int8_t, int16_t, int32_t>;
...
TYPED_TEST(StridedSliceOpTest, In1D_Int32End) {
  for (bool constant_tensors : {true, false}) {
    ...
    StridedSliceOpModel<TypeParam> m({32768}, {1}, {1}, {1}, casted_values, {0},
                                     {32768}, {1}, 0, 0, 0, 0, 0,
                                     constant_tensors);
    ASSERT_EQ(m.Invoke(), kTfLiteOk);
    EXPECT_THAT(m.GetOutputShape(), ElementsAreArray({32768}));
    EXPECT_THAT(m.GetOutput(), ElementsAreArray(casted_values));
  }
}
...
TYPED_TEST(StridedSliceOpTest, In1D_EvenLenStride2) {
  for (bool constant_tensors : {true, false}) {
    ...
    std::vector<TypeParam> input_data = CastVector<TypeParam>({1, 2});
...
TYPED_TEST(StridedSliceOpTest, In1D_ShrinkAxisMask1_NegativeSlice) {
  for (bool constant_tensors : {true, false}) {
    ...
    std::vector<TypeParam> input_data = CastVector<TypeParam>({0, 1, 2, 3});
...
TYPED_TEST(StridedSliceOpTest, In5D_Identity) {
  for (bool constant_tensors : {true, false}) {
    ...
    EXPECT_THAT(m.GetOutput(),
                ElementsAreArray({1, 2, 3, 4, 9, 10, 11, 12}));
  }
}
...
TYPED_TEST(StridedSliceOpTest, OneOneOne) {
  for (bool constant_tensors : {true, false}) {
    ...
    std::vector<TypeParam> input_data = CastVector<TypeParam>({2});
