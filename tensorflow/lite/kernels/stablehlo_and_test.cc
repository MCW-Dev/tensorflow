...
TEST(StablehloElementwise, AndInt8){
  AndOpModel model({TensorType_INT8, {1,3,1}},
                   {TensorType_INT8, {1,3,1}}, {TensorType_INT8, {}});
...
TEST(StablehloElementwise, AndInt16){
  AndOpModel model({TensorType_INT16, {1,1,3}},
                   {TensorType_INT16, {1,1,3}}, {TensorType_INT16, {}});
...
TEST(StablehloElementwise, AndBool){
  AndOpModel model({TensorType_BOOL, {2,1,2,1}},
    ...
    ASSERT_EQ(model.Invoke(), kTfLiteOk);
    EXPECT_THAT(model.GetOutput<bool>(), ElementsAre(0, 0, 0, 1));
}
