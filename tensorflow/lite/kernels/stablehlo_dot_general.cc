...
TfLiteStatus EvalImp(const TfLiteTensor* lhs, const TfLiteTensor* rhs,
                     TfLiteTensor* output) {
  const int batch_size = lhs->dims->data[0];
  const int n = lhs->dims->data[1];
  const int p = rhs->dims->data[1];
  const int m = lhs->dims->data[2];
  ...
  for (int batch = 0; batch < batch_size; ++batch) {
    EigenMatrixMapRowMajorConst eigen_lhs(
        GetTensorData<DataType>(lhs) + batch * n * p, n, p);
    EigenMatrixMapColMajorConst eigen_rhs(
        GetTensorData<DataType>(rhs) + batch * m * p, p, m);
    EigenMatrixMapRowMajorMutable eigen_dst(
        GetTensorData<DataType>(output) + batch * n * m, n, m);
    if (m == 1) {
...
TfLiteStatus Eval(TfLiteContext* context, TfLiteNode* node) {
  ...
  if (data_type == kTfLiteInt8) {
    return EvalImp<int8_t>(lhs, rhs, output);
  } else if (data_type == kTfLiteBFloat16) {
    return EvalImp<tensorflow::bfloat16>(lhs, rhs, output);
  } else if (data_type == kTfLiteFloat16) {
    return EvalImp<Eigen::half>(lhs, rhs, output);
  } else if (data_type == kTfLiteFloat32) {
    return EvalImp<float>(lhs, rhs, output);
  } else if (data_type == kTfLiteInt16) {
    return EvalImp<int16_t>(lhs, rhs, output);
  } else if (data_type == kTfLiteInt32) {
    return EvalImp<int32_t>(lhs, rhs, output);
  } else {
