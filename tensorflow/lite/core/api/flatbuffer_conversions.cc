...
TfLiteStatus ParseStablehloDotGeneral(const Operator* op,
  ...
  if (schema_params != nullptr) {
    ...
    TF_LITE_ENSURE_STATUS(LoadAttr(params->lhs_batching_dimensions,
                                   sizeof(params->lhs_batching_dimensions),
                                   schema_params->lhs_batching_dimensions(),
                                   "lhs_batching_dimensions"));
    TF_LITE_ENSURE_STATUS(LoadAttr(params->rhs_batching_dimensions,
                                   sizeof(params->rhs_batching_dimensions),
                                   schema_params->rhs_batching_dimensions(),
    ...
    if (schema_params->lhs_batching_dimensions()->size() !=
        schema_params->rhs_batching_dimensions()->size()) {
      TF_LITE_REPORT_ERROR(error_reporter,
                           "'stablehlo.dot_general' operation parameter array "
                           "sizes are not consistent.");
      return kTfLiteError;
    }
    if (schema_params->lhs_contracting_dimensions()->size() !=
